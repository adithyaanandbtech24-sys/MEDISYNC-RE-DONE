...
public class MedicalDataParser {
    ...
    private static let labMetadata: [LabTestMetadata] = [
        ...
        // Glucose & Diabetes
        LabTestMetadata(name: "Fasting Glucose", aliases: ["glucose", "fasting glucose", "blood sugar", "fbs", "sugar (f)"], unit: "mg/dL", category: "Glucose", normalRange: "70-100 mg/dL", pattern: #"\d+\.?\d*"#),
        LabTestMetadata(name: "HbA1c", aliases: ["hba1c", "a1c", "glycated hb", "glyco hb", "glycosylated haemoglobin"], unit: "%", category: "Glucose", normalRange: "< 5.7%", pattern: #"\d+\.?\d*"#),
    ...
    private static func extractBloodGroup(from text: String) -> String? {
        ...
        if let regex = try? NSRegularExpression(pattern: pattern, options: .caseInsensitive),
           let match = regex.firstMatch(in: text, range: NSRange(text.startIndex..., in: text)),
           let range = Range(match.range(at: 1), in: text) {
            var bg = String(text[range]).uppercased().replacingOccurrences(of: " ", with: "")
            return bg
        }
        return nil
    }
    ...
    public static func parseLineByLine(from text: String, existingResults: [LabResultModel]) -> [LabResultModel] {
        ...
        let knownTests: [(name: String, aliases: [String], unit: String, category: String, organ: String)] = [
            ...
            // GLUCOSE/DIABETES
            ("Fasting Glucose", ["fasting glucose", "glucose fasting", "blood glucose", "fbs"], "mg/dL", "Glucose", "Pancreas"),
            ("HbA1c", ["hba1c", "glycated hemoglobin", "glycosylated"], "%", "Glucose", "Pancreas"),
        ...
        for line in lines {
            ...
            for test in knownTests {
                ...
                if matched && !existingNames.contains(test.name.lowercased()) {
                    ...
                    // Pattern: find a standalone number (not part of reference range like "30 - 110")
                    ...
                    if let regex = try? NSRegularExpression(pattern: valuePattern, options: .caseInsensitive) {
                        ...
                        if let match = regex.firstMatch(in: lowercasedLine, range: range),
                           let valueRange = Range(match.range(at: 1), in: lowercasedLine) {
                            ...
                            if let rawValue = Double(valueStr) {
